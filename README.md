...
### 📖 Все классы по умолчанию должны быть final
Если класс не подразумевает наследование, а значит для наследования нет покрытия тестами, то такой класс должен быть обозначен как `final`. Если класс не обозначен как `final`, то должен быть обозначен как `abstract`, или должны быть тесты на поведение класса при наследовании.

Если такое обозначение когда-нибудь понадобится убрать, то это будет обратно совместимое изменение, которое не повлечёт за собой явных поломок в коде, использующем этот класс. Обозначить класс как `final` если он уже где-то используется при наследовании, как правило, невозможно, потому лучше обозначить его как `final` сразу.
