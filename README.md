...
Если ожидается возврат лишь списка, а не конкретно массива, то допустимо использовать генератор, в phpdoc указывая подсказкой как массив каких элементов выглядит этот генератор. 
