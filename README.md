...
# Содержание
  ...
  19. [Service, Entity, Engine](#Service,-Entity,-Engine)
  20. [Особенности Pull Request (PR)](#Особенности-Pull-Request-(PR))
  21. [Работа с компонентами](#Работа-с-компонентами)
  22. [Работа с шаблонами](#Работа-с-шаблонами)
  23. [Работа с патчами](#Работа-с-патчами)
  24. [Работа с литералами](#Работа-с-литералами)
  25. [Работа с условиями](#Работа-с-условиями)
  26. [Работа с тернарными операторами](#Работа-с-тернарными-операторами)
  27. [Про тесты](#Про-тесты)
  28. [Использование chain-объектов](#Использование-chain-объектов)
  29. [Работа со скриптами](#Работа-со-скриптами)
...
## **Service, Entity, Engine**
### Все классы делятся на Service, Entity и Engine
### Service — это любой класс, содержащий только логику, без состояния
### Entity — это любой класс, содержащий данные (состояние), без логики
Однако `Entity` может содержать методы. Бизнес логикой не считаются разного рода геттеры и сеттеры. Также `Entity` может содержать методы для вспомогательного представления или интерпретации своих внутренних данных. Например, если есть объект `Transaction`, у которого есть 2 константы `TYPE_CHARGE` и `TYPE_TOPUP`, то надо сделать у этого объекта 2 метода: `isTopup()` и `isCharge()`, которые будут проверять `$this->_type` на соответствие константам. Это требуется, чтобы не дублировать сравнение по всему коду проекта
### Engine — это абстрактная реализация класса какого-то компонента или фичи, направленная на удобное добавление новых реализаций
Например, это может быть абстрактный `Engine` платежного шлюза. Каждый новый платежный шлюз в таком случае добавляется наследованием от абстрактного. `Engine` может включать в себя как логику, так и данные. Его главная задача — облегчить добавление новых реализаций. Новая реализация должна добавляться только в одном месте. Код проекта не должен знать конкретные реализации этого `Engine` и работать исключительно с абстракцией.
